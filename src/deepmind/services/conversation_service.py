...
class ConversationService:
    ...
    async def send_message(
        ...
        Args:
            ...
            model: "deepseek" or "gpt4o" - which LLM to use
        """
        # Store user message
        ...
        # Check for dev-scaffold trigger (Google Drive search) - only for DeepSeek
        scaffold_query = None
        if model == "deepseek":
            scaffold_query = await self.deepseek_client.analyze_for_dev_scaffold(user_content)
            if scaffold_query:
        ...
        if model == "gpt4o":
            ...
            async for delta in self.openai_client.stream_chat(messages=context_messages):
                full_response += delta
                yield delta
            model_name = self.cfg.openai.model if hasattr(self.cfg, 'openai') else "gpt-4o"
        else:
            # Stream from DeepSeek (default)
            async for delta in self.deepseek_client.chat_completion_stream(messages=context_messages):
                full_response += delta
                yield delta
